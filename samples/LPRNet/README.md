...
## 1. 简介
本例程可在EA65xx平台上进行LPRNet车牌识别模型的推理，支持加载浮点和量化模型，适用于对CCPD数据集进行分割处理后的车牌图片。模型来源：
https://github.com/sirius-ai/LPRNet_Pytorch 。具体描述可参考[模型转换](docs/LPRNet_Export_Guide.md)。
...
├── dataset.txt
├── lprnet.onnx
├── lprnet_config_fp16.json
└── lprnet_float16.nb
```
下载的数据包括：
```
datasets/
├── test  # CCPD数据集中1000张处理后的车牌图片
├── lable.txt # 数据集1000张图片所对应的TXT格式标注信息
└── lable.json # 数据集1000张图片所对应的JSON格式标注信息
```
### 3.2 模型编译
如果您不编译模型，直接使用下载的数据集和模型，可跳过本小节。
源模型需要编译成nb才能在EA65xx平台上运行，可以使用onnx模型进行编译转换。具体可参考[模型转换](docs/LPRNet_Export_Guide.md)。同时，您需要准备用于测试的数据集，如果量化模型，还要准备用于量化的数据集。
...
```bash
convert_model build --output_dir ./lprnet --config models/lprnet_config_fp16.json
```
执行上述命令后，会在lprnet目录下生成转换好的模型文件。

## 4. 例程测试
cpp目录下提供了C++例程以供参考使用，具体情况说明如下：
| 序号 | C++例程 | 说明 |
| ---- | ---- | ---- |
| 1 | lprnet_det_soc | 使用ta_opencv前处理，taruntime推理 |
...
make 
```
编译完成后，会在目录下生成lprnet_det_soc。
### 4.3 推理测试
需将交叉编译生成的可执行文件及所需的模型、测试数据拷贝到EA65XX平台测试。
#### 4.3.1 参数说明
可执行程序默认有一套参数，请注意根据实际情况进行传参，具体参数说明如下：
```
Usage: lprnet_det_soc [params] 
        ...
        -i, --input_dir (value:datasets/img)
                test image dir
        -m, --model (value:lprnet_fp16.nb)
                model file
```
### 4.3.2 图片测试
图片测试实例如下，支持对整个图片文件夹和单张图片进行测试。
```bash
./lprnet_det_soc -i=datasets/test/京PL3N67.jpg -m=lprnet_float16.nb
```
单张图片测试结束后，会打印预测的车牌识别结果、推理时间等信息。
```bash
./lprnet_det_soc -i=datasets/test -a=datasets/label.txt -m=lprnet_float16.nb
```
...
./lprnet_det_soc -i=datasets/test -a=datasets/label.txt -m=lprnet_float16.nb
```
该指令会根据提供的数据集中的labels.txt文件和推理的结果来计算模型的精度，执行成功后会在命令行中打印精度测试结果。用户如需测试个人数据集可以对应修改-a参数和-i参数。
