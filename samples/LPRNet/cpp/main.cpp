...
CmdParams parser(int argc, char** argv){
    const char * keys = 
        ...
        "{model m      | lprnet_fp16.nb  | model file}"
...
bool readLabelFile(const std::string& file_path, std::map<std::string, std::string>& label_map) {
...
void compareLabelFiles(const std::string& file1, const std::string& file2) {
    ...
    std::cout << "Read file1 done." << std::endl;
    if (!readLabelFile(file2, map2)) return;
    std::cout << "Read file2 done." << std::endl;
    ...
    for (const auto& pair : map1) {
        ...
        std::string label1 = pair.second;

        if (map2.find(path) != map2.end()) {
            // 条目在两个文件中都存在
            std::string label2 = map2[path];
            if (label1 == label2) {
                correct_top1++;
            } 
        }
...
int main(int argc, char* argv[]){
    ...
    LPRNET lprnet(nnrt_ctx,model_file);
    ...
    if(info.st_mode & S_IFREG){ // process single image
        ...
        lprnet.Classify(images, results);
        total++;
        std::cout << image_dir << " pred: " << results[0] << std::endl;
        ...
        lprnet.Classify(images, results);
        // std::cout<< " pred: " << results[0] << std::endl;
        
        {
            std::ofstream out_file("result.txt");
            for(int i = 0; i < results.size(); i++){
                out_file << image_names[i] << " " << results[i]<< "\n";
            }
            out_file.close();
        }
