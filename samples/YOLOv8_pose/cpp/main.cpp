    ...
    else if (info.st_mode & S_IFDIR)
    {
        ...
        for (int i = 0; i < images.size(); i++)
        {
            for (auto result : results[i])
            {
                ...
                person_json["score"] = result.score;

                nlohmann::json keypoints_json = nlohmann::json::array();
                for (int k = 0; k < 17; k++)
                {
                    keypoints_json.push_back(result.kpts[k].x);
                    keypoints_json.push_back(result.kpts[k].y);
                    keypoints_json.push_back(result.kpts[k].score);
                }
                person_json["keypoints"] = keypoints_json;
                results_json.push_back(person_json);
            }
            std::string output_image_path = "results/images/" + image_names[i];
            if (config.save_draw && config.draw_obj)
