...
## 2. 主要步骤 
### 2.1 导出torchscript模型
​Pytorch模型在编译前要经过`torch.jit.trace`，trace后的模型才能使用taNNTC编译nb。
### 2.2 导出onnx模型
如果使用taNNTC编译模型，则必须先将Pytorch模型导出为onnx模型。官方仓库提供了模型导出接口，可以直接使用它导出onnx模型：
```bash
import torch
import torch.onnx
