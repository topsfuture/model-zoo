...
inline std::filesystem::path script_path = std::filesystem::current_path();
inline std::string vocab_path = (script_path / "tokenizer/vocab.txt").string();
...
inline std::vector<std::wstring> wsplit(const std::wstring &ws, const std::wstring &delimiters) {
...
inline std::wstring wjoin(const std::vector<std::wstring> &tokens, const std::wstring &delimiter) {
...
inline static std::string normalize_nfd(const std::string& s) {
    std::string ret;
...
inline static bool isStripChar(const wchar_t& ch) {
    return stripChar.find(ch) != std::wstring::npos;
}

inline static std::wstring strip(const std::wstring& text) {
...
inline static std::vector<std::wstring> split(const std::wstring& text) {
    std::vector<std::wstring>  result = wsplit(text, stripChar);
    return result;
}

inline static std::vector<std::wstring> whitespaceTokenize(const std::wstring& text) {
    std::wstring rtext = strip(text);
    if (rtext.empty()) return std::vector<std::wstring>();
    return split(text);
}

inline static std::wstring convertToUnicode(const std::string& text) {
    size_t i = 0;
    std::wstring ret;
    while (i < text.size()) {
        wchar_t codepoint;
...
inline static std::string convertFromUnicode(const std::wstring& wText) {
    char dst[64];
    std::string ret;
    for (auto ch : wText) {
...
inline static std::shared_ptr<Vocab> loadVocab(const std::string& vocabFile) {
...
inline BasicTokenizer::BasicTokenizer(bool doLowerCase) 
    : mDoLowerCase(doLowerCase) {
}

inline std::wstring BasicTokenizer::cleanText(const std::wstring& text) const {
    std::wstring output;
...
inline bool BasicTokenizer::isControol(const wchar_t& ch) const {
    if (ch== L'\t' || ch== L'\n' || ch== L'\r') return false;
...
inline bool BasicTokenizer::isWhitespace(const wchar_t& ch) const {
    if (ch== L' ' || ch== L'\t' || ch== L'\n' || ch== L'\r') return true;
    auto cat = utf8proc_category(ch);
...
inline bool BasicTokenizer::isPunctuation(const wchar_t& ch) const {
...
inline bool BasicTokenizer::isChineseChar(const wchar_t& ch) const {
...
inline std::wstring BasicTokenizer::tokenizeChineseChars(const std::wstring& text) const {
    std::wstring output;
    for (auto& ch : text) {
        if (isChineseChar(ch)) {
            output += L' ';
...
inline std::wstring BasicTokenizer::runStripAccents(const std::wstring& text) const {
...
inline std::vector<std::wstring> BasicTokenizer::runSplitOnPunc(const std::wstring& text) const {
    size_t i = 0;
...
inline std::vector<std::wstring> BasicTokenizer::tokenize(const std::string& text) const {
...
inline WordpieceTokenizer::WordpieceTokenizer(const std::shared_ptr<Vocab> vocab, const std::wstring& unkToken, size_t maxInputCharsPerWord)
    : mVocab(vocab),
    mUnkToken(unkToken),
    mMaxInputCharsPerWord(maxInputCharsPerWord) {
}

inline std::vector<std::wstring> WordpieceTokenizer::tokenize(const std::wstring& text) const {
...
inline FullTokenizer::FullTokenizer(const std::string vocabFile, bool doLowerCase) : 
...
inline std::vector<int> FullTokenizer::encode(const std::string& text,int max_length, bool add_special_token,
    bool padding ) const{
...
inline std::string FullTokenizer::decode( std::vector<size_t>& input_ids){
    std::string s ="";
...
inline std::vector<std::wstring> FullTokenizer::tokenize(const std::string& text) const {
    std::vector<std::wstring> splitTokens;
...
inline std::vector<int> FullTokenizer::convertTokensToIds(const std::vector<std::wstring>& text) const {
