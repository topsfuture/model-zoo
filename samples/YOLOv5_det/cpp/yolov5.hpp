...
class YOLOV5 {
  ...
  void generate_proposals_yolov5_generic_clip(
    int stride, void* feat, DataType dtype, int32_t zp, float scale,
    float prob_threshold, YoloV5BoxVec& objects,int letterbox_cols, 
    int letterbox_rows, const std::vector<int>& anchors);

  public:
  std::vector<taconn_profile_t> hardware_time;
  YOLOV5(std::shared_ptr<ta_runtime_context> context,std::string model_file,
        float conf_thresh_i = 0.25,float nms_thresh_i = 0.45);
  ~YOLOV5();
  void enableProfile(TimeStamp *ts);
  void draw_objects(const cv::Mat& bgr, const YoloV5BoxVec& objects, 
                  const std::string& output_name, double fontScale = 0.5, int thickness = 1);
  int Init();
  int get_output_num();
  int Detect(std::vector<cv::Mat>& input_images, std::vector<YoloV5BoxVec>& box);
  int deInit();
};
