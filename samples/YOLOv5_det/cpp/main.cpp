...
CmdParams parser(int argc, char** argv){
    const char * keys = 
        ...
        "{model m      | yolov5s_int8.nb  | model file}"
...
int main(int argc, char* argv[]){
    ...
    YOLOV5 yolov5(nnrt_ctx,model_file,conf_thresh,nms_thresh);
    ...
    yolov5.enableProfile(ts);
    int total = 0;

    std::vector<cv::Mat> images;
    std::vector<std::string> image_names;
    std::vector<YoloV5BoxVec> results;
    ...
    if(info.st_mode & S_IFREG){
        ...
        yolov5.Detect(images,results);
        total++;
        yolov5.draw_objects(image, results[0], "result.jpg");
        ...
        for(int i = 0; i < images.size(); i++){
            ...
            if(config.save_draw)
                yolov5.draw_objects(images[i], results[i], output_image_path);
