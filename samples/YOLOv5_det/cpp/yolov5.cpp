...
YOLOV5::YOLOV5(std::shared_ptr<ta_runtime_context> context,std::string model_file,float conf_thresh_i ,float nms_thresh_i ):nnrt_context(context),model_name(model_file),
...
size_t YOLOV5::get_element_num(const taconn_inout_attr_t input_attr) {
    size_t size = 1;
...
void YOLOV5::mat_to_tensor(const cv::Mat& mat, uint8_t* tensor) {
    ...
    //     uint8_t* channel_ptr = tensor + c * total_pixels;
    //     for (int h = 0; h < height; ++h) {
    //         const uint8_t* row_ptr = mat.data + (h * width * channels);
    //         for (int w = 0; w < width; ++w) {
    //             *channel_ptr++ = row_ptr[w * channels + c];
    //         }
    //     }
    // }
    const int height = mat.rows;
...
int YOLOV5::preprocess_image(const cv::Mat& src, cv::Mat& dst,PreprocessParams& params){
    params.src_size = src.size();
    params.ratio = std::min(static_cast<float>(input_image_h) / src.rows,
                           static_cast<float>(input_image_w) / src.cols);
    ...
    params.top = (input_image_h - new_h) / 2;
    params.bottom = input_image_h - new_h - params.top;
    params.left = (input_image_w - new_w) / 2;
    params.right = input_image_w - new_w - params.left;
...
void YOLOV5::normalize_and_quantize(uint8_t* src, void* dst, size_t num_elements, 
...
int YOLOV5::pre_process(cv::Mat &images){
    ...
    for(int i = 0; i < input_num; i++){
        ...
        if (ins_attr[i].quant_format == taconn_qnt_type_e::TACONN_QNT_TYPE_NONE &&
            ...
            input_tensor_uint8 = (uint8_t*)malloc(sizeof(uint8_t) * input_image_h * input_image_w * 3);
...
void YOLOV5::generate_proposals_yolov5_generic_clip(
    int stride, void* feat, DataType dtype, int32_t zp, float scale,
    float prob_threshold, YoloV5BoxVec& objects,
    int letterbox_cols, int letterbox_rows, const std::vector<int>& anchors)
{
    /* clip at last conv layer, out tensor shape (1,3*85,h,w) */
    int feat_w = letterbox_cols / stride;
    int feat_h = letterbox_rows / stride;
    int cls_num = 80;
    int anchor_num = anchors.size() / 2; // 每个锚点有宽和高两个值
    
    // 获取反量化函数
    DequantizeFunc dequant = dequantize_funcs[static_cast<int>(dtype)];
    
    // 每个锚点预测的数值数量 (x, y, w, h, obj_conf + cls_num)
    const int values_per_anchor = 5 + cls_num;
    
    // 特征图的总元素数
    const size_t col_len = feat_h * feat_w; // 每个通道的元素数
    
    for (int a = 0; a < anchor_num; a++) {
        int grid_offset_1 = a * values_per_anchor * col_len;
        
        for (int h = 0; h < feat_h; h++) {
            int grid_offset_3 = h * feat_w ;
            
            for (int w = 0; w < feat_w; w++) {
                int grid_offset = grid_offset_1 + grid_offset_3 + w;
                
                // 获取目标置信度
                float obj_conf = dequant(feat, grid_offset + 4 * col_len, zp, scale);
                obj_conf = g_sigmoid(obj_conf);
                ...
                for (int c = 0; c < cls_num; c++) {
                    float cls_score = dequant(feat, grid_offset + (5 + c) * col_len, zp, scale);
                    cls_score = g_sigmoid(cls_score);
                ...
                if (final_score < prob_threshold) continue;
                
                // 解码边界框
                float dx = dequant(feat, grid_offset + 0 * col_len, zp, scale);
                float dy = dequant(feat, grid_offset + 1 * col_len, zp, scale);
                float dw = dequant(feat, grid_offset + 2 * col_len, zp, scale);
                float dh = dequant(feat, grid_offset + 3 * col_len, zp, scale);
                ...
                // 计算边界框坐标
                float x0 = pred_cx - pred_w / 2;
                float y0 = pred_cy - pred_h / 2;
                float x1 = pred_cx + pred_w / 2;
                float y1 = pred_cy + pred_h / 2;
...
void g_qsort_descent_inplace(YoloV5BoxVec& faceobjects, int left, int right)
{
    int i = left;
    int j = right;
...
void g_qsort_descent_inplace(YoloV5BoxVec& faceobjects)
{
...
void YOLOV5::draw_objects(const cv::Mat& bgr, const YoloV5BoxVec& objects, const std::string& output_name, double fontScale, int thickness)
{
...
inline float g_intersection_area(const YoloV5Box& a, const YoloV5Box& b)
{
...
void g_nms_sorted_bboxes_class_aware(const YoloV5BoxVec& objects, 
...
void inverse_coordinates(YoloV5Box& box, const PreprocessParams& params) {
    // Step 1: 去除填充偏移（浮点计算）
    box.x -= params.left;
    box.y -= params.top;
    ...
    // Step 5: 重新计算宽高（确保非负）
    box.width = clamped_x1 - box.x;
    box.height = clamped_y1 - box.y;
    box.width = std::max(box.width, 0.0f);
    box.height = std::max(box.height, 0.0f);
}
void YOLOV5::post_process(const cv::Mat& image, 
                  const PreprocessParams& pre_params,
                  YoloV5BoxVec& objects) 
{
    YoloV5BoxVec proposals;
    // YOLOv5的锚点（三个输出层）
    ...
    for (int i = 0; i < output_num; ++i) {
        ...
        generate_proposals_yolov5_generic_clip(
            stride,
            output_data,
            dtype,
            zp,
...
size_t YOLOV5::calculate_buffer_size(taconn_data_format_t format, size_t element_count) {
    switch(format) {
        // 32位类型 (4字节)
...
int YOLOV5::Detect(std::vector<cv::Mat>& input_images, std::vector<YoloV5BoxVec>& box){
...
{ 
    ...
    input_image_h = ins_attr[0].dim_size[0];
    input_image_w = ins_attr[0].dim_size[1];
    ...
    // 4. set output
    /* default float32, select in ['int8','uint8', 'float16', 'float32'] */
    // Output = {};
