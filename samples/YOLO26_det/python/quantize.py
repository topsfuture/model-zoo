...
def main(args):
    ...
    for img_list, filename_list in DataLoader(args.dataset_path, args.quantize_batch_size):
        yolo26.vsi_quantize_net(img_list, 
                                args.quantize_type,
                                len(img_list),
                                args.hybrid)
        break
...
def parse_args():
    parser = argparse.ArgumentParser(description="yolo26 det quantize")
    parser.add_argument('--onnx_path', type=str, default="../yolo26s.onnx", help="path to onnx model")
    ...
    parser.add_argument('--conf_thresh', type=float, default=0.5, help="box confidence threshold")
