...
static const std::vector<cv::Scalar> COCO_COLORS = {
    ...
    {128, 207, 0, 255}, {128, 37, 0, 255}, {128, 0, 207, 255}, {128, 94, 0, 255}, {128, 0, 255, 263}, 
    ...
    {128, 56, 255, 0}, {128, 255, 94, 0}, {128, 255, 263, 0}, {128, 0, 132, 255}, {128, 255, 0, 132}, 
    {128, 255, 170, 0}, {128, 255, 0, 188}, {128, 263, 255, 0}, {128, 245, 0, 255}, {128, 263, 0, 255}, 
    {128, 255, 188, 0}, {128, 0, 263, 255}, {128, 255, 0, 0}, {128, 0, 56, 255}, {128, 255, 0, 263}, 
...
YOLO26Detector::YOLO26Detector() : initialized_(false), input_height_(640), input_width_(640) {
    ctx_.nnrt_context = 0;
...
bool YOLO26Detector::detect_and_save(const cv::Mat& image,
    ...
    // 后处理
    if (ts_) ts_->start();
    post_process(image, pre_params, conf_thresh, objects, output_path);
...
void YOLO26Detector::preprocess_image(const cv::Mat& src, cv::Mat& dst, PreprocessParams& params) {
...
void YOLO26Detector::post_process(const cv::Mat& image, 
                                 const PreprocessParams& pre_params,
                                 float conf_thresh, 
                                 std::vector<Object>& objects,
                                 const std::string& output_image_path) {
    std::vector<Object> proposals;
    auto generate_proposals_from_concat = [&](void* feat,
        ...
        for (int i = 0; i < num_boxes; ++i) {
            ...
            for (int c = 0; c < cls_num; ++c) {
                size_t offset = static_cast<size_t>(box_channels + c) * num_boxes + i;
            ...
            obj.box.left = x0;
            obj.box.top = y0;
            obj.box.width = std::max(0.0f, x1 - x0);
            obj.box.height = std::max(0.0f, y1 - y0);
    ...
    auto get_affine_quant_params = [](const taconn_inout_attr_t& attr,
                                      int32_t& zp,
                                      float& scale) {
        zp = 0;
        scale = 1.0f;
    ...
    auto generate_proposals_from_split = [&](void* box_feat,
                                             uint32_t box_data_format,
                                             int32_t box_zp,
                                             float box_scale,
                                             void* cls_feat,
                                             uint32_t cls_data_format,
                                             int32_t cls_zp,
                                             float cls_scale,
                                             float prob_threshold,
                                             std::vector<Object>& out_objects,
        ...
        for (int i = 0; i < box_num; ++i) {
            ...
            for (int c = 0; c < cls_num; ++c) {
                size_t offset = static_cast<size_t>(c) * box_num + i;
                float score = dequantize_value(cls_feat, offset, cls_data_format, cls_zp, cls_scale);
            ...
            float x0 = dequantize_value(box_feat, static_cast<size_t>(0) * box_num + i,
                                        box_data_format, box_zp, box_scale);
            float y0 = dequantize_value(box_feat, static_cast<size_t>(1) * box_num + i,
                                        box_data_format, box_zp, box_scale);
            float x1 = dequantize_value(box_feat, static_cast<size_t>(2) * box_num + i,
                                        box_data_format, box_zp, box_scale);
            float y1 = dequantize_value(box_feat, static_cast<size_t>(3) * box_num + i,
                                        box_data_format, box_zp, box_scale);
    ...
    if (ctx_.output_num == 1) {
        void* output_data = ctx_.output_buffer[0].data;
        uint32_t data_format = ctx_.outs_attr[0].data_format;

        int32_t zp = 0;
        float scale = 1.0f;
        get_affine_quant_params(ctx_.outs_attr[0], zp, scale);

        generate_proposals_from_concat(output_data, ctx_.outs_attr[0], data_format, zp, scale,
                                       conf_thresh, proposals, input_width_, input_height_);
    } else if (ctx_.output_num == 2) {

        //output[0]: [1,4 8400] output[1]: [1,80 8400]， box_index:0 cls_index: 1
            int32_t box_zp = 0;
            int32_t cls_zp = 0;
            float box_scale = 1.0f;
            float cls_scale = 1.0f;
            get_affine_quant_params(ctx_.outs_attr[0], box_zp, box_scale);
            get_affine_quant_params(ctx_.outs_attr[1], cls_zp, cls_scale);

            generate_proposals_from_split(ctx_.output_buffer[0].data,
                                          ctx_.outs_attr[0].data_format,
                                          box_zp,
                                          box_scale,
                                          ctx_.output_buffer[1].data,
                                          ctx_.outs_attr[1].data_format,
                                          cls_zp,
                                          cls_scale,
                                          conf_thresh,
                                          proposals,
    ...
    const int topk = std::min(300, static_cast<int>(proposals.size()));
    objects.resize(topk);
...
// 反量化函数重载实现
float YOLO26Detector::dequantize(float value, int32_t, float) {
    return value;
}

float YOLO26Detector::dequantize(uint16_t value, int32_t, float) {
    // float16 转 float32
...
float YOLO26Detector::dequantize(uint8_t value, int32_t zp, float scale) {
    return ((float)value - (float)zp) * scale;
}

float YOLO26Detector::dequantize(int8_t value, int32_t zp, float scale) {
    return ((float)value - (float)zp) * scale;
}

// 通用反量化函数
float YOLO26Detector::dequantize_value(void* data, size_t idx, uint32_t data_format, 
...
void YOLO26Detector::inverse_coordinates(bbox& box, const PreprocessParams& params) {
    if (params.ratio <= 0.0f || params.src_size.width <= 0 || params.src_size.height <= 0) {
    ...
    float x0 = (box.left - static_cast<float>(params.left)) * inv_ratio;
    float y0 = (box.top - static_cast<float>(params.top)) * inv_ratio;
    float x1 = (box.left + box.width - static_cast<float>(params.left)) * inv_ratio;
    float y1 = (box.top + box.height - static_cast<float>(params.top)) * inv_ratio;

    x0 = std::max(0.0f, std::min(x0, static_cast<float>(params.src_size.width)));
    y0 = std::max(0.0f, std::min(y0, static_cast<float>(params.src_size.height)));
    x1 = std::max(0.0f, std::min(x1, static_cast<float>(params.src_size.width)));
    y1 = std::max(0.0f, std::min(y1, static_cast<float>(params.src_size.height)));

    box.left = x0;
    box.top = y0;
    box.width = std::max(0.0f, x1 - x0);
    box.height = std::max(0.0f, y1 - y0);
}

void YOLO26Detector::draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects, 
...
size_t YOLO26Detector::get_element_num(const taconn_inout_attr_t& input_attr) {
    size_t size = 1;
...
size_t YOLO26Detector::calculate_buffer_size(taconn_data_format_t format, size_t element_count) {
    switch(format) {
...
size_t YOLO26Detector::element_size(uint32_t data_format) {
    switch (data_format) {
...
uint16_t YOLO26Detector::float32_to_float16(float value) {
...
void YOLO26Detector::normalize_and_quantize(uint8_t* src, void* dst, size_t num_elements, 
                                           const taconn_inout_attr_t& attr) {
...
void YOLO26Detector::mat_to_tensor(const cv::Mat& mat, uint8_t* tensor) {
...
CmdParams parse_arguments(int argc, char** argv) {
    const char* keys = 
        ...
        "{model m      | yolo26s_int8.nb  | model file           }"
...
void YOLO26Detector::print_taconn_inout_attr(const taconn_inout_attr_t& attr) {
    std::cout << "--------------------------------------------------------------" << "\n";
...
void YOLO26Detector::enableProfile(TimeStamp *ts) {
    ts_ = ts;
  }
