...
int run_single_image_inference(const CmdParams& config) {
    ...
    if (!detector.detect_and_save(image, config.output, objects, config.conf_thresh)) {
...
int run_batch_inference(const CmdParams& config) {
    ...
    std::vector<YOLO26Detector::BatchResult> results;
    int total_objects = 0;
    ...
    for (const auto& image_file : image_files) {
        ...
        YOLO26Detector::BatchResult result;
        ...
        std::vector<Object> objects;
        if (!detector.detect_and_save(image, output_path, objects, config.conf_thresh)) {
