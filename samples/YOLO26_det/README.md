...
## 1. 简介
本例程可在ea65xx平台上进行YOLO26检测模型的推理，支持加载浮点和量化模型，适用于COCO等常见数据集。模型来源：
...
├── yolo26s.onnx
├── yolo26s_int8.nb
├── yolo26s_config_fp16.json
└── yolo26s_config_int8.json
```
下载的数据包括：
```
datasets/
...
源模型需要编译成nb才能在EA65xx平台上运行，可以使用onnx模型或者torchscripts模型进行编译转换。具体可参考[模型转换](docs/YOLO26_Export_Guide.md)。同时，您需要准备用于测试的数据集，如果量化模型，还要准备用于量化的数据集。
...
```bash
convert_model build --output_dir ../yolo26 --config models/yolo26s_config_fp16.json
```
执行上述命令后，会在yolo26目录下生成转换好的模型文件。
- 生成INT8 nb
同上，请注意修改config.json中的源模型路径、源模型框架、模型预处理参数和输入大小shape等参数，如：
```bash
convert_model build --output_dir ../yolo26 --config models/yolo26s_config_int8.json
```
...
| 序号 | C++例程 | 说明 |
| ---- | ---- | ---- |
| 1 | yolo26s_det_soc | 使用taOpenCV前处理，taRuntime推理 |
...
```bash
./yolo26s_det_soc --input=./datasets/val2017_1000/ --model=yolo26s_float16.nb --conf_thresh=0.001 --output=yolo26s_fp16.json --batch=true
```
测试结束后，会将预测的图片保存在results/下，预测的结果保存在yolo26s_fp16.json中，同时会打印预测结果、推理时间等信息。

### 4.4 模型精度评估

通过以上步骤的，在板端可以获得用于精度验证的json文件，我们需要将其拷贝至pc端。精度验证在pc端进行，验证的脚本为YOLO26_det/python/utils/coco_eval.py，真实标注json文件在datasets下（instances_val2017_1000.json），具体执行路径请用户自行指定。
