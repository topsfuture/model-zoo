...
MOBILENETV2::MOBILENETV2(std::shared_ptr<ta_runtime_context> context,std::string model_file) : nnrt_context(context) ,model_name(model_file){
  
  std::cout << "--------------------------------------" << std::endl;
  std::cout << "Mobilenetv2 create nnrt_context" << std::endl;
...
size_t MOBILENETV2::get_element_num(const taconn_inout_attr_t input_attr) {
    size_t size = 1;
...
size_t MOBILENETV2::calculate_buffer_size(taconn_data_format_t format, size_t element_count) {
    switch(format) {
        // 32 bits (4bytes)
...
bool MOBILENETV2::Init() {
    uint32_t model_version = 0;
    taconn_input_output_num_t num;
    taconn_inout_attr_t input_attr;
    taconn_inout_attr_t output_attr;
    memset(&input_attr, 0, sizeof(taconn_inout_attr_t));
    memset(&output_attr, 0, sizeof(taconn_inout_attr_t));
...
int MOBILENETV2::preprocess_image(const cv::Mat& src, cv::Mat& dst) {
    cv::Mat img;
...
int MOBILENETV2::pre_process(cv::Mat &images) {
...
int MOBILENETV2::post_process(std::vector<cv::Mat> &images, std::vector<std::pair<int, float>> &results){
    ...
    float sum_exp = 0.0f;
    float max_val = *std::max_element(dequant.begin(), dequant.end());
    for (int i = 0; i < DEFAULT_CLASS_NUM; i++) {
        dequant[i] = std::exp(dequant[i]-max_val);
        sum_exp += dequant[i];
    }
    for (size_t i = 0; i < DEFAULT_CLASS_NUM; ++i) {
...
int MOBILENETV2::Classify(std::vector<cv::Mat>& input_images, std::vector<std::pair<int, float>> &results){
...
void MOBILENETV2::enableProfile(TimeStamp *ts) {
  ts_ = ts;
}