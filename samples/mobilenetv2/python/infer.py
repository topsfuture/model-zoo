...
def save_json(results_list: list, output_dir: str, quantize_type:str,  dataset_path: str):
    ...
    json_name = os.path.split("mobilenetv2.onnx")[-1] + '_' + quantize_type + "_" + os.path.split(dataset_path)[-1] + "_python_result.json"
...
def parse_args():
    ...
    parser.add_argument("--onnx_path", type=str, default="../models/mobilenetv2-12.onnx", help="path of model")
...
def main():
    ...
    if args.quantize_type in ['int8', 'uint8', 'float16', 'bfloat16', 'int16']:
        mobilenetv2.load_q_net(args.quantize_type)
    
    mobilenetv2.nn.build_inference_session(mobilenetv2.acuity_net)
    ...
    for img_list, filename_list in DataLoader(args.dataset_path, args.batch_size):
        outputs = mobilenetv2.infer_image(img_list)
        # postprocess(outputs)
        # logging.info(f"outputs dtype: {outputs.dtype}, shape: {outputs.shape}")
