...
#define USE_CUT_MODEL 0
YOLOV12::YOLOV12(std::shared_ptr<ta_runtime_context> context,std::string model_file,float conf_thresh_i ,float nms_thresh_i ,bool use_multi_label):nnrt_context(context),model_name(model_file),
    ...
    output_num = 0;
    initialized_ = false;
    use_multi_label_nms = use_multi_label;
    std::cout<< "conf_thresh: "<< conf_thresh << ", nms_thresh: "<< nms_thresh << std::endl;
}
...
size_t YOLOV12::get_element_num(const taconn_inout_attr_t input_attr) {
    size_t size = 1;
...
void YOLOV12::mat_to_tensor(const cv::Mat& mat, uint8_t* tensor) {
...
float YOLOV12::dequantize(float value, int32_t, float) {
    return value;
}

float YOLOV12::dequantize(uint16_t value, int32_t, float) {
    // float16 转 float32
...
float YOLOV12::dequantize(uint8_t value, int32_t zp, float scale) {
    return ((float)value - (float)zp) * scale;
}

float YOLOV12::dequantize(int8_t value, int32_t zp, float scale) {
    return ((float)value - (float)zp) * scale;
}
float YOLOV12::dequantize_value(void* data, size_t idx, uint32_t data_format, 
...
int YOLOV12::preprocess_image(const cv::Mat& src, cv::Mat& dst,PreprocessParams& params){
...
static uint16_t f32_to_f16(float value) {
    ...
    if (exponent < -14) {  
        ...
        mantissa >>= shift;
        if ((mantissa & 0x1000) && ((mantissa & 0x1FFF) != 0x1000 || (mantissa & 0x2000))) {
            mantissa += 0x1000;
        }
        return sign | (mantissa >> 13);
    }

    uint32_t fp16_exponent = (exponent + 15) << 10;
    uint32_t fp16_mantissa = mantissa >> 13;

    if ((mantissa & 0x1000) && ((mantissa & 0x1FFF) != 0x1000 || (fp16_mantissa & 1))) {
        fp16_mantissa += 1;
        if (fp16_mantissa == 0x400) {  
            fp16_exponent += 0x400;
            fp16_mantissa = 0;
        }
    }

    return sign | fp16_exponent | fp16_mantissa;
}
void YOLOV12::normalize_and_quantize(uint8_t* src, void* dst, size_t num_elements, 
...
int YOLOV12::pre_process(cv::Mat &images){
...
size_t YOLOV12::element_size(uint32_t data_format) {
    switch (data_format) {
...
float YOLOV12::softmax_with_stride_generic(void* src, float* dst, int length, 
...
void YOLOV12::generate_proposals_yolov12_generic(
    int stride, void* feat, int data_format, int32_t zp, float scale,
    float prob_threshold, YoloV12BoxVec& objects,
        ...
        for (int w = 0; w < feat_w; w++) {
            ...
            for (int c = 0; c < 4; c++) {
                ...
                pred_ltrb[c] = softmax_with_stride_generic(
                    static_cast<char*>(feat) + offset * element_size(data_format),
...
void g_qsort_descent_inplace(YoloV12BoxVec& faceobjects, int left, int right)
{
    int i = left;
    int j = right;
...
void g_qsort_descent_inplace(YoloV12BoxVec& faceobjects)
{
...
void YOLOV12::draw_objects(const cv::Mat& bgr, const YoloV12BoxVec& objects, const std::string& output_name, double fontScale, int thickness)
{
    ...
    {
        const YoloV12Box& obj = objects[i];
        if(obj.class_id >= 80)
            continue;
...
inline float g_intersection_area(const YoloV12Box& a, const YoloV12Box& b)
{
...
void g_nms_sorted_bboxes_class_aware(const YoloV12BoxVec& objects, 
...
{   /*class aware nms*/
    ...
    for (auto& pair : class_indices)
    {
        ...
        for (int i = 0; i < indices.size(); i++)
        {
            ...
            const YoloV12Box& a = objects[idx_i];
            
            int keep = 1;
            for (int j = 0; j < class_picked.size(); j++)
            {
                const int idx_j = class_picked[j];
                const YoloV12Box& b = objects[idx_j];
...
void inverse_coordinates(YoloV12Box& box, const PreprocessParams& params) {
    // Step 1: 去除填充偏移（浮点计算）
    box.x -= params.left;
    box.y -= params.top;
...
void YOLOV12::post_process(const cv::Mat& image, 
                  const PreprocessParams& pre_params,
                  YoloV12BoxVec& objects) 
{
    YoloV12BoxVec proposals;
    ...
    for (int i = 0; i < output_num; ++i) {
        ...
        int32_t zp = 0;
        float scale = 1.0f;
        int data_format = outs_attr[i].data_format;
        ...
        generate_proposals_yolov12_generic(
            stride,
            output_data,
            data_format,
            zp,
        ...
        for (int i = 0; i < box_num; i++) {
            ...
            int coord_offset = box_index + 4 * offset;
            if(use_multi_label_nms){
            // multilabel
                for (int j = 0; j < 80; j++) {
                    int last_offset = j * offset + coord_offset;
                    // float cur_value = cls_conf[j * offset];
                    float cur_value = dequantize_value(output_data,last_offset,data_format,zp, scale);
                    if (cur_value > conf_thresh) {
                        YoloV12Box box;
                        box.score = cur_value;
                        box.class_id = j;

                        float centerX = dequantize_value(output_data,box_index,data_format,zp, scale);
                        float centerY = dequantize_value(output_data,box_index + 1 * offset,data_format,zp, scale);
                        float width = dequantize_value(output_data, box_index + 2 * offset,data_format,zp, scale);
                        float height = dequantize_value(output_data,box_index + 3 * offset,data_format,zp, scale);
                        

                        int c = 0;
                        box.x = centerX - width / 2 + c;
                        box.y = centerY - height / 2 + c;
                        box.width = width;
                        box.height = height;
...
size_t YOLOV12::calculate_buffer_size(taconn_data_format_t format, size_t element_count) {
    switch(format) {
        // 32位类型 (4字节)
...
int YOLOV12::Detect(std::vector<cv::Mat>& input_images, std::vector<YoloV12BoxVec>& box){
    for(int i = 0; i < input_images.size(); i++){
        ...
        YoloV12BoxVec detected_objects;
...
void YOLOV12::enableProfile(TimeStamp *ts) {
  ts_ = ts;
}