...
using YoloV12BoxVec = std::vector<YoloV12Box>;

class YOLOV12 {
    
    float conf_thresh;
    float nms_thresh;
    bool use_multi_label_nms = true;
  ...
  private:
    ...
    void generate_proposals_yolov12_generic(
            int stride, void* feat, int dtype, int32_t zp, float scale,
  ...
  public:
    ...
    YOLOV12(std::shared_ptr<ta_runtime_context> context,std::string model_file,
            float conf_thresh_i,float nms_thresh_i,bool use_multi_label);
    ~YOLOV12();
    void enableProfile(TimeStamp *ts);
    void draw_objects(const cv::Mat& bgr, const YoloV12BoxVec& objects, 
                    const std::string& output_name, double fontScale = 0.5, int thickness = 1);
    int Init();
    size_t element_size(uint32_t data_format);
    int Detect(std::vector<cv::Mat>& input_images, std::vector<YoloV12BoxVec>& box);
    int deInit();
};
