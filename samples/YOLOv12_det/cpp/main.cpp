...
CmdParams parser(int argc, char** argv){
    ...
    config.use_multi_label = parser.get<bool>("use_multi_label");
...
int main(int argc, char* argv[]){
    ...
    auto use_multi_label = config.use_multi_label;
    struct stat info;
    ...
    YOLOV12 yolov12(nnrt_ctx,model_file,conf_thresh,nms_thresh,use_multi_label);
    ...
    yolov12.enableProfile(ts);
    int total = 0;

    std::vector<cv::Mat> images;
    std::vector<std::string> image_names;
    std::vector<YoloV12BoxVec> results;
    ...
    if(info.st_mode & S_IFREG){
        ...
        yolov12.Detect(images,results);
        total++;
        yolov12.draw_objects(image, results[0], "result.jpg");
        ...
        for(int i = 0; i < images.size(); i++){
            ...
            if(config.save_draw)
                yolov12.draw_objects(images[i], results[i], output_image_path);
