...
class PostProcess:
    ...
    def  __call__(self, preds_batch, org_size_batch, ratios_batch, txy_batch):
        """
        post-processing
        :param preds_batch:     list of predictions in a batch [[pred1, pred2, ...]]
        ...
        #     # 1 output
        #     print(f"preds_batch[0] type:{len(preds_batch[0])}")
        #     dets = np.concatenate(preds_batch)
        #     print(f"\ndets shape: {dets.shape}\n\n\n\n\n\n")
        # else:
        #     print('preds_batch type: '.format(type(preds_batch)))
        #     raise NotImplementedError
        if isinstance(preds_batch, np.ndarray) and preds_batch.shape == (1, 84, 8400):
    # 维度转置：(1,84,8400) → (1,8400,84) → 展平为 (8400,84)
            dets = preds_batch.transpose(0, 2, 1)
        else:
            raise NotImplementedError(f"Unsupported output shape: {preds_batch.shape}")
...
class pseudo_torch_nms:
    ...
    def non_max_suppression(self,
        ...
        Params:
            prediction  - torch.Tensor - predictions shape (batch_size, 8400, 84)
        Returns:
        ...
        for xi, x in enumerate(prediction):  # image index, image inference
            ...
            x = x[xc[xi]]  # pic det after max_conf filtering
