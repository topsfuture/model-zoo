...
## 1. 简介
本例程可在ea65xx平台上进行YOLOv12检测模型的推理，支持加载浮点和量化模型，适用于COCO等常见数据集。模型来源：
...
源模型需要编译成nb才能在EA65xx平台上运行，可以使用onnx模型或者torchscripts模型进行编译转换。具体可参考[模型转换](docs/YOLOv12_Export_Guide.md)。同时，您需要准备用于测试的数据集，如果量化模型，还要准备用于量化的数据集。
...
```bash
convert_model build --output_dir ../yolov12 --config models/yolov12s_config_fp16.json
```
执行上述命令后，会在yolov12目录下生成转换好的模型文件。

## 4. 例程测试
cpp目录下提供了C++例程以供参考使用，具体情况说明如下：
| 序号 | C++例程 | 说明 |
| ---- | ---- | ---- |
| 1 | yolov12s_det_soc | 使用taOpenCV前处理，taRuntime推理 |
...
```bash
./yolov12s_det_soc -m=yolov12s_float16.nb -c=0.001 -n=0.7 -i=datasets/val2017_1000/
```
测试结束后，预测的结果保存在results/output.json中，同时会打印预测结果、推理时间等信息。

### 4.4 模型精度评估

通过以上步骤的，在板端可以获得用于精度验证的json文件（results/output.json），我们需要将其拷贝至pc端。精度验证在pc端进行，验证的脚本为python/coco_eval.py，真实标注json文件在datasets下（instances_val2017_1000.json），具体执行路径请用户自行指定。

```bash
pip install pycocotools
python3 coco_eval.py -g 用户路径/datasets/instances_val2017_1000.json -r 用户路径/results/output.json
```
执行成功后，会打印精度测试结果。
