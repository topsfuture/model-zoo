...
float PostProcessor::PolygonScoreAcc(std::vector<cv::Point> contour, cv::Mat pred) 
{
  int width = pred.cols;
  int height = pred.rows;
...
std::vector<std::vector<int>> PostProcessor::OrderPointsClockwise(std::vector<std::vector<int>> pts){
...
OCRBoxVec PostProcessor::FilterTagDetRes(std::vector<std::vector<std::vector<int>>> boxes,
                                        cv::Mat &input_img) {
    int oriimg_h = input_img.rows;
    int oriimg_w = input_img.cols;
    OCRBoxVec ocrboxes;
